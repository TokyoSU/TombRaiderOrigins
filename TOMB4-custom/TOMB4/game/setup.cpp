...
static void ObjectObjects()
{
	...
	for (int i = PUZZLE_ITEM1; i <= QUEST_ITEM8; i++)
	{
		obj = &objects[i];
	...
	for (int i = PISTOLS_ITEM; i <= MEMCARD_SAVE_INV_ITEM; i++)
	{
		obj = &objects[i];
		obj->initialise = InitialisePickUp;
		obj->control = AnimatingPickUp;
	...
	obj = &objects[WATERSKIN1_EMPTY_ITEM];
	obj->initialise = InitialisePickUp;
	obj->control = AnimatingPickUp;
	obj->collision = PickUpCollision;
	obj->save_position = 1;
	...
	obj = &objects[WATERSKIN2_EMPTY_ITEM];
	obj->initialise = InitialisePickUp;
	obj->control = AnimatingPickUp;
	obj->collision = PickUpCollision;
	obj->save_position = 1;
	...
	obj = &objects[CLOCKWORK_BEETLE_ITEM];
	obj->initialise = 0;
...
//	obj->collision = empty func here;
	...
	for (int i = PUSHABLE_BLOCK1; i <= PUSHABLE_BLOCK5; i++)
	{
		obj = &objects[i];
...
void InitialiseLara()
{
	...
	lara.holster_right = LARA_HOLSTERS_PISTOLS;
	lara.holster_left = LARA_HOLSTERS_PISTOLS;
	lara.location = -1;
