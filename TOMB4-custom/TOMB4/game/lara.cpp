...
long UseInventoryItems(ITEM_INFO* item)
{
	...
	{
		if (in_use >= WATERSKIN1_EMPTY_ITEM && in_use <= WATERSKIN2_5_ITEM)
		{
			item->item_flags[2] = LARA_WATER_MESH;

			if (in_use != WATERSKIN1_3_ITEM && in_use != WATERSKIN2_5_ITEM && (LaraNodeUnderwater[3] || LaraNodeUnderwater[6]))
			{
				if (in_use >= WATERSKIN1_3_ITEM)
			...
			else if (in_use != WATERSKIN1_EMPTY_ITEM && in_use != WATERSKIN2_EMPTY_ITEM)
			{
				if (in_use > WATERSKIN1_3_ITEM)
				{
		...
		else if (in_use == CLOCKWORK_BEETLE_ITEM)
		{
