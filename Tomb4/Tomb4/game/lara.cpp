...
void(*lara_control_routines[118])(ItemInfo* item, COLL_INFO* coll) =
{
	lara_as_walk,
	lara_as_run,
	lara_as_stop,
...
void(*lara_collision_routines[118])(ItemInfo* item, COLL_INFO* coll) =
{
	lara_col_walk,
	lara_col_run,
	lara_col_stop,
...
static void TiltHer(ItemInfo* item, long rad, long height)
{
	FloorInfo* floor;
	FVECTOR plane;
	long wy[4];
...
static long IsValidHangPos(ItemInfo* item, COLL_INFO* coll)
{
	short angle;
...
long LaraTestHangOnClimbWall(ItemInfo* item, COLL_INFO* coll)
{
	...
	if (!LaraTestClimbPos(item, coll->r, coll->r, bounds[2], bounds[3] - bounds[2], &shift) ||
		!LaraTestClimbPos(item, coll->r, -coll->r, bounds[2], bounds[3] - bounds[2], &shift))
		return 0;

	result = LaraTestClimbPos(item, coll->r, 0, bounds[2], bounds[3] - bounds[2], &shift);
...
long LaraTestClimbStance(ItemInfo* item, COLL_INFO* coll)
{
	...
	if (LaraTestClimbPos(item, coll->r, coll->r + 120, -700, 512, &shift_r) != 1)
		return 0;

	if (LaraTestClimbPos(item, coll->r, -120 - coll->r, -700, 512, &shift_l) != 1)
		return 0;
