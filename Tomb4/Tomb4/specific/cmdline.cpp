...
INT_PTR CALLBACK DXSetupDlgProc(HWND dlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if (message == WM_COMMAND)
	{
		...
		case 1000:
			...
			{
				nDDDevice = (long)SendMessage(GetDlgItem(dlg, 1000), CB_GETCURSEL, 0, 0);
				InitD3DDevice(dlg, GetDlgItem(dlg, 1003));
			}
		...
		case 1003:
			...
			if (((wParam >> 16) & 0xFFFF) == CBN_SELCHANGE)
			{
				nD3DDevice = (long)SendMessage(GetDlgItem(dlg, 1003), CB_GETCURSEL, 0, 0);
				InitResolution(dlg, GetDlgItem(dlg, 1004), 1);
			}
...
{
	ShowCursor(TRUE);
	long ret = DialogBox(App.hInstance, MAKEINTRESOURCE(109), 0, DXSetupDlgProc);
	ShowCursor(FALSE);

	if (ret == -1)
	{
		MessageBox(0, "Unable To Initialise Dialog", "Setup error", 0);
		return false;
	}
