...
display_rots rots[34] =
{
	...
	{PISTOL_ITEM, 0x4000, 0, -1},
	{SHOTGUN_ITEM, 0x5000, -0x2000, -1},
	{MAGNUM_ITEM, 0x4000, 0, -1},
	{HARPOON_ITEM, -0x3000, 0, -1},
	{ROCKET_GUN_ITEM, 0x1000, -0x2000, -1},
	{GRENADE_GUN_ITEM, -0x3000, 0, -1},
	{MP5_ITEM, -0x3000, 0, -1},
	{DESERTEAGLE_ITEM, -0x2500, -0x2000, -1},
	{UZI_ITEM, 0x4000, 0, -1},
	{FLAREBOX_ITEM, 0, 0, -1},

	{SHOTGUN_AMMO_ITEM, 0xE38, 0, -1},
	{DESERTEAGLE_AMMO_ITEM, 0x1000, 0, -1},
	{UZI_AMMO_ITEM, -0x2000, 0, -1},
	{MAGNUM_AMMO_ITEM, -0x2000, 0, -1},
	{HARPOON_AMMO_ITEM, 0xE38, 0, -1},
	{MP5_AMMO_ITEM, 0x1000, 0, -1},
	{ROCKET_AMMO_ITEM, 0, 0, -1},
	{GRENADE_AMMO_ITEM, 0, 0, -1},

	{MEDI_ITEM, 0, 0, -1},
	{BIGMEDI_ITEM, 0, 0, -1},

	{ICON_PICKUP1_ITEM, 0, 0, -1},
	{ICON_PICKUP2_ITEM, 0, 0, -1},
	{ICON_PICKUP3_ITEM, 0, 0, -1},
	{ICON_PICKUP4_ITEM, 0, 0, -1},
};

static void GetRots(short obj_num, short& x, short& z, short& ignoredMesh)
{
	for (int i = 0; i < 34; i++)
	{
		if (rots[i].objectNumber == obj_num)
		{
			x = rots[i].rotX;
			z = rots[i].rotZ;
			ignoredMesh = rots[i].ignoredMeshID;
			return;
		}
	}
...
// NOTE: a bit more complicated now, but no more need to edit mesh to draw the pickup correctly,
// Now all meshes is drawed, if still you need a mesh to be hide, you can set the ignoreMeshID in the rots[] to the mesh you want to hide (without crash).
static void DrawPickup(short obj_num)
{
	short rotx = 0, rotz = 0, ignoredMesh = -1;
	float x = float(phd_winxmin + phd_winxmax - GetFixedScale(75));
	float y = float(phd_winymax + phd_winymin - GetFixedScale(75));
	GetRots(obj_num, rotx, rotz, ignoredMesh);
	...
	auto* pickupObj = &objects[obj_num];
	short* frmptr[2]{};
	long rate = 0;
	auto frac = GetFramesWithObject(pickupObj, 0, 0, frmptr, &rate);
	if (frmptr[0] == nullptr)
		return;

	auto* meshpp = &meshes[pickupObj->mesh_index];
	auto* bone = &bones[pickupObj->bone_index];
