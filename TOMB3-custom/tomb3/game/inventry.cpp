...
void SelectMeshes(INVENTORY_ITEM* item)
{
	...
	else if (item->object_number == STOPWATCH_OPTION)
	{
...
void DrawInventoryItem(INVENTORY_ITEM* item)
{
	...
	if (item->object_number == STOPWATCH_OPTION)
	{
		s = savegame.timer / 30;
		m = -91 * (s % 3600) / 5;
		h = -91 * (s / 12) / 5;
	...
	do
	{
		...
		case RNG_OPEN:
			...
			if (inputDB & IN_SELECT)
			{
				...
				else if (item->object_number >= PISTOL_OPTION && item->object_number <= GRENADE_OPTION || item->object_number == MAGNUM_OPTION)
					SoundEffect(SFX_MENU_GUNS, NULL, SFX_ALWAYS);
				else if (item->object_number == PHOTO_OPTION)
					SoundEffect(SFX_MENU_POLAROID, NULL, SFX_ALWAYS);
				else
