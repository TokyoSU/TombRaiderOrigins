...
long GF_LoadScriptFile(const char* name)
{
	...
	ipistol_option.itemText = GF_GameStrings[GT_PISTOLS];
	imagnum_option.itemText = GF_GameStrings[GT_MAGNUMS];
	...
	ipistolammo_option.itemText = GF_GameStrings[GT_PISTOLCLIPS];
	imagnumammo_option.itemText = GF_GameStrings[GT_MAGNUMSCLIPS];
...
void GF_ModifyInventory(long level, long type)
{
	...
	if (Inv_RequestItem(SHOTGUN_ITEM))
	{
		if (type)
		{
			...
			for (int i = 0; i < GF_SecretInvItems[ADDINV_SHOTGUN_AMMO]; i++)
				AddDisplayPickup(SHOTGUN_AMMO_ITEM);
		}
		else
	...
	if (Inv_RequestItem(DESERTEAGLE_ITEM))
	{
		if (type)
		{
			lara.deserteagle.ammo += 10 * GF_SecretInvItems[ADDINV_DESERTEAGLE_AMMO];

			for (int i = 0; i < GF_SecretInvItems[ADDINV_DESERTEAGLE_AMMO]; i++)
				AddDisplayPickup(DESERTEAGLE_AMMO_ITEM);
		}
		else
			lara.deserteagle.ammo += 10 * GF_Add2InvItems[ADDINV_DESERTEAGLE_AMMO];
	}
	else if (!type && GF_Add2InvItems[ADDINV_DESERTEAGLE] || type == 1 && GF_SecretInvItems[ADDINV_DESERTEAGLE])
	{
		start->got_deserteagle = 1;
		Inv_AddItem(DESERTEAGLE_ITEM);

		if (type)
		{
			AddDisplayPickup(DESERTEAGLE_ITEM);
			lara.deserteagle.ammo += 10 * GF_Add2InvItems[ADDINV_DESERTEAGLE_AMMO];

			for (int i = 0; i < GF_SecretInvItems[ADDINV_DESERTEAGLE]; i++)
				AddDisplayPickup(DESERTEAGLE_AMMO_ITEM);
		}
		else
			lara.deserteagle.ammo += 10 * GF_Add2InvItems[ADDINV_DESERTEAGLE_AMMO];
	...
	else if (type)
	{
		for (int i = 0; i < GF_SecretInvItems[ADDINV_DESERTEAGLE_AMMO]; i++)
		{
			Inv_AddItem(DESERTEAGLE_AMMO_ITEM);
			AddDisplayPickup(DESERTEAGLE_AMMO_ITEM);
		}
	}
	else
	{
		for (int i = 0; i < GF_Add2InvItems[ADDINV_DESERTEAGLE_AMMO]; i++)
			Inv_AddItem(DESERTEAGLE_AMMO_ITEM);
	}
	...
	if (Inv_RequestItem(MAGNUM_ITEM))
	{
		if (type)
		{
			lara.magnums.ammo += 30 * GF_SecretInvItems[ADDINV_MAGNUMS_AMMO];

			for (int i = 0; i < GF_SecretInvItems[ADDINV_MAGNUMS_AMMO]; i++)
				AddDisplayPickup(MAGNUM_AMMO_ITEM);
		}
		else
			lara.magnums.ammo += 30 * GF_Add2InvItems[ADDINV_MAGNUMS_AMMO];
	}
	else if (!type && GF_Add2InvItems[ADDINV_MAGNUMS] || type == 1 && GF_SecretInvItems[ADDINV_MAGNUMS])
	{
		start->got_magnum = 1;
		...
		if (type)
		{
			...
			for (int i = 0; i < GF_SecretInvItems[ADDINV_MAGNUMS]; i++)
				AddDisplayPickup(MAGNUM_AMMO_ITEM);
		}
		else
			lara.magnums.ammo += 30 * GF_Add2InvItems[ADDINV_MAGNUMS_AMMO];
	...
	else if (type)
	{
		for (int i = 0; i < GF_SecretInvItems[ADDINV_MAGNUMS_AMMO]; i++)
		{
			Inv_AddItem(MAGNUM_AMMO_ITEM);
			AddDisplayPickup(MAGNUM_AMMO_ITEM);
		}
	}
	else
	{
		for (int i = 0; i < GF_Add2InvItems[ADDINV_MAGNUMS_AMMO]; i++)
			Inv_AddItem(MAGNUM_AMMO_ITEM);
	}
	...
	if (Inv_RequestItem(MP5_ITEM))
	{
		if (type)
		{
			lara.m16.ammo += 60 * GF_SecretInvItems[ADDINV_MP5_AMMO];

			for (int i = 0; i < GF_SecretInvItems[ADDINV_MP5_AMMO]; i++)
				AddDisplayPickup(MP5_AMMO_ITEM);
		}
		else
			lara.m16.ammo += 60 * GF_Add2InvItems[ADDINV_MP5_AMMO];
	}
	else if (!type && GF_Add2InvItems[ADDINV_MP5] || type == 1 && GF_SecretInvItems[ADDINV_MP5])
	{
		start->got_mp5 = 1;
	...
	else if (type)
	{
		for (int i = 0; i < GF_SecretInvItems[ADDINV_MP5_AMMO]; i++)
		{
			Inv_AddItem(MP5_AMMO_ITEM);
			AddDisplayPickup(MP5_AMMO_ITEM);
		}
	}
	else
	{
		for (int i = 0; i < GF_Add2InvItems[ADDINV_MP5_AMMO]; i++)
			Inv_AddItem(MP5_AMMO_ITEM);
	}
