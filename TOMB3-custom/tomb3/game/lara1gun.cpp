...
void draw_shotgun(long weapon_type)
{
	...
	if (lara.weapon_item == NO_ITEM)
	{
		...
		if (weapon_type == LG_ROCKET)
			item->anim_number = objects[ROCKET_GUN_ANIM].anim_index + 1;
...
void FireM16(long running)
{
	...
	{
		weapons[MP5_ANIM].shot_accuracy = 2184;	/*Technically LG_ROCKET*/
		weapons[MP5_ANIM].damage = 1;
	}
	else
	{
		weapons[MP5_ANIM].shot_accuracy = 728;
		weapons[MP5_ANIM].damage = 3;
	}
