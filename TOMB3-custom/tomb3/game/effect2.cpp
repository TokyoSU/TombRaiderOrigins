...
void TriggerGunSmoke(long x, long y, long z, long xv, long yv, long zv, long initial, long weapon, long shade)
{
	...
	if ((weapon == LG_PISTOLS || weapon == LG_MAGNUM || weapon == LG_DESERTEAGLE || weapon == LG_UZIS) && sptr->dR > 64)
	{
		sptr->dR = 64;
		sptr->dG = 64;
		sptr->dB = 64;
...
void TriggerGunShell(short lr, long objNum, long weapon)
{
	...
	TriggerGunSmoke(pos.x, pos.y, pos.z, 0, 0, 0, 0, weapon, weapon == LG_SHOTGUN ? 24 : 16);
