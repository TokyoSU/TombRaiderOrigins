...
WEAPON_INFO weapons[NUM_WEAPONS] =
{
	...
	{
		-10920, 10920, -10920, 10920,		//LG_MAGNUM
		-30940, 10920, -14560, 14560,
		-10920, 30940, -14560, 14560,
		1820,
		1456,
...
void InitialiseNewWeapon()
{
	...
	case LG_UZIS:
	case LG_MAGNUM:
		lara.right_arm.frame_base = objects[PISTOL_ANIM].frame_base;
		lara.left_arm.frame_base = objects[PISTOL_ANIM].frame_base;
...
void LaraGun()
{
	...
	case LG_READY:
		...
		if (lara.pistols.ammo && input & IN_ACTION)
			lara.mesh_ptrs[HEAD] = meshes[objects[UZI_ANIM].mesh_index + HEAD];
		else
			lara.mesh_ptrs[HEAD] = meshes[objects[LARA].mesh_index + HEAD];
