...
void CreateStartInfo(long level)
{
	...
	if (Inv_RequestItem(PISTOL_ITEM))
		pInfo->got_pistols = 1;
	else
		pInfo->got_pistols = 0;
	...
	{
		pInfo->got_deserteagle = 1;
		pInfo->deserteagle_ammo = (ushort)lara.deserteagle.ammo;
	}
	else
	{
		pInfo->got_deserteagle = 0;
		pInfo->deserteagle_ammo = ushort(Inv_RequestItem(DESERTEAGLE_AMMO_ITEM) * 10);
	}
	...
	{
		pInfo->got_magnum = 0;
		pInfo->magnum_ammo = ushort(Inv_RequestItem(MAGNUM_AMMO_ITEM) * 30);
	}
	...
	{
		pInfo->got_shotgun = 0;
		pInfo->shotgun_ammo = ushort(Inv_RequestItem(SHOTGUN_AMMO_ITEM) * 12);
	}
	...
	{
		pInfo->got_mp5 = 1;
		pInfo->mp5_ammo = (ushort)lara.m16.ammo;
	}
	else
	{
		pInfo->got_mp5 = 0;
		pInfo->mp5_ammo = ushort(Inv_RequestItem(MP5_AMMO_ITEM) * 60);
	}
...
void WriteSG(void* pointer, long size)
{
	char* data;
	SGcount += size;
	if (SGcount >= sizeof(savegame.buffer))
