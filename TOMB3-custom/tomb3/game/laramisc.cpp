...
void LaraInitialiseMeshes(long level_number)
{
	...
	if (start->gun_type != LG_UNARMED)
	{
		...
		{
			lara.mesh_ptrs[THIGH_L] = meshes[objects[DESERTEAGLE_ANIM].mesh_index + THIGH_L];
			lara.mesh_ptrs[THIGH_R] = meshes[objects[DESERTEAGLE_ANIM].mesh_index + THIGH_R];
		}
		else if (start->gun_type == LG_UZIS)
		{
			lara.mesh_ptrs[THIGH_L] = meshes[objects[UZI_ANIM].mesh_index + THIGH_L];
			lara.mesh_ptrs[THIGH_R] = meshes[objects[UZI_ANIM].mesh_index + THIGH_R];
		}
		else if (start->gun_type == LG_MAGNUM)
		{
			lara.mesh_ptrs[THIGH_L] = meshes[objects[MAGNUM_ANIM].mesh_index + THIGH_L];
			lara.mesh_ptrs[THIGH_R] = meshes[objects[MAGNUM_ANIM].mesh_index + THIGH_R];
		}
		else
		{
	...
	if (start->gun_type == LG_MP5)
		lara.back_gun = MP5_ANIM;
	else if (start->gun_type == LG_HARPOON)
		lara.back_gun = HARPOON_GUN_ANIM;
	else if (start->gun_type == LG_ROCKET)
		lara.back_gun = ROCKET_GUN_ANIM;
...
void UseItem(short object_number)
{
	...
	case MAGNUM_ITEM:
	case MAGNUM_OPTION:
		lara.request_gun_type = LG_MAGNUM;
		break;
...
void InitialiseLara(long type)
{
	...
	if (type == 4)
	{
		...
		lara.mesh_ptrs[THIGH_L] = meshes[objects[PISTOL_ANIM].mesh_index + THIGH_L];
		meshes[objects[PISTOL_ANIM].mesh_index + THIGH_L] = tmp;

		tmp = lara.mesh_ptrs[THIGH_R];
		lara.mesh_ptrs[THIGH_R] = meshes[objects[PISTOL_ANIM].mesh_index + THIGH_R];
		meshes[objects[PISTOL_ANIM].mesh_index + THIGH_R] = tmp;
...
void InitialiseLaraInventory(long level)
{
	...
	if (start->got_deserteagle)
	{
		Inv_AddItem(DESERTEAGLE_ITEM);
		lara.deserteagle.ammo = start->deserteagle_ammo;
		GlobalItemReplace(DESERTEAGLE_ITEM, DESERTEAGLE_AMMO_ITEM);
	}
	else
	{
		ammo = start->deserteagle_ammo / 10;

		for (int i = 0; i < ammo; i++)
			Inv_AddItem(DESERTEAGLE_AMMO_ITEM);

		lara.deserteagle.ammo = 0;
	}
	...
	{
		Inv_AddItem(MP5_ITEM);
		lara.m16.ammo = start->mp5_ammo;
		GlobalItemReplace(MP5_ITEM, MP5_AMMO_ITEM);
	}
	else
	{
		ammo = start->mp5_ammo / 60;
